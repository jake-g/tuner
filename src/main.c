...
void computeSecondOrderLowPassParameters(float srate, float f, float* a,
                                         float* b);
float processSecondOrderFilter(float x, float* mem, const float* a,
                               const float* b);
void signalHandler(int signum);
int findNearestNote(float freq, const float* freqTable,
                    const char** noteNameTable, float* nearestNotePitch,
                    int tableSize);
...
int main(void) {
  ...
  err = Pa_OpenStream(&stream, &inputParameters, NULL, SAMPLE_RATE, FFT_SIZE,
                      paClipOff, NULL, NULL);
  if (err != paNoError) goto error;
  ...
  while (running) {
    ...
    int nearestNoteIndex = findNearestNote(freq, freqTable, noteNameTable,
                                           &nearestNotePitch, FFT_SIZE / 2);

    const char* nearestNoteName =
        (nearestNoteIndex != -1) ? noteNameTable[nearestNoteIndex] : "";
    ...
    if (nearestNoteIndex != -1) {
      ...
      if (fabsf(centsSharp) >
          0.01f) {  // Only print if significantly different from zero
        if (centsSharp > 0) {
      ...
      if (fabsf(centsSharp) < 0.01f || centsSharp >= 0) {
        ...
        for (int i = 0; i < (int)(chars + centsSharp); ++i) printf(" ");
        for (int i = (chars + centsSharp < 0 ? 0 : (int)(chars + centsSharp));
             i < chars; ++i)
          printf("=");
      }
      ...
      if (fabsf(centsSharp) > 0.01f) {
        for (int i = 0; i < chars && i < (int)centsSharp; ++i) printf("=");
      }
...
float processSecondOrderFilter(float x, float* mem, const float* a,
                               const float* b) {
  float ret =
      b[0] * x + b[1] * mem[0] + b[2] * mem[1] - a[0] * mem[2] - a[1] * mem[3];
  mem[1] = mem[0];
  mem[0] = x;
  mem[3] = mem[2];
  mem[2] = ret;
